...
# if (NOT ANTLR4CPP_CMAKE_ARGS)
#   set(ANTLR4CPP_CMAKE_ARGS )
# endif()
...
if(NOT EXISTS "${ANTLR4CPP_LOCAL_ROOT}" AND NOT EXISTS "${ANTLR4CPP_LOCAL_REPO}")
message(STATUS "Antlr4Cpp downloading runtime with git")
...
message(STATUS "Antlr4Cpp using local runtime file")
message(STATUS "ANTLR4CPP_LOCAL_ROOT : ${ANTLR4CPP_LOCAL_ROOT}")
message(STATUS "ANTLR4CPP_LOCAL_REPO : ${ANTLR4CPP_LOCAL_REPO}")

# Use locally supplied runtime 
ExternalProject_ADD(
  #--External-project-name------
  antlr4cpp
  # ...
  ...
  # INSTALL_DIR        ${ANTLR4CPP_LOCAL_ROOT}/build-install

  #--Download step--------------
  URL                 ${ANTLR4CPP_LOCAL_REPO}
  # URL_HASH            ${ANTLR4CPP_LOCAL_REPO_HASH}
  #--Patch step----------
  # PATCH_COMMAND sh -c "cp <SOURCE_DIR>/scripts/CMakeLists.txt <SOURCE_DIR>"
  #--Configure step-------------
  CONFIGURE_COMMAND  ${CMAKE_COMMAND} -DCMAKE_BUILD_TYPE=Release -DANTLR4CPP_JAR_LOCATION=${ANTLR4CPP_JAR_LOCATION} -DBUILD_SHARED_LIBS=ON -BUILD_TESTS=OFF -DCMAKE_INSTALL_PREFIX:PATH=<INSTALL_DIR> -DCMAKE_SOURCE_DIR:PATH=<SOURCE_DIR>/runtime/src <SOURCE_DIR>
  LOG_CONFIGURE ON
  #--Build step-----------------
  # BUILD_COMMAND ${CMAKE_MAKE_PROGRAM}
  LOG_BUILD ON
...
# Silence Attributes warnting
foreach(include_file ${ANTLR4CPP_INCLUDE_DIRS})
...
# set(antlr4_static ${INSTALL_DIR}/lib/libantlr4-runtime.a)
...
macro(antlr4cpp_process_grammar
  ...
  add_custom_target("antlr4cpp_generation_${antlr4cpp_project_namespace}"
    ...
    COMMAND
    "${Java_JAVA_EXECUTABLE}" -jar "${ANTLR4CPP_JAR_LOCATION}" -Dlanguage=Cpp -no-visitor -listener -o "${ANTLR4CPP_GENERATED_SRC_DIR}/${antlr4cpp_project_namespace}" -package ${antlr4cpp_project_namespace} "${antlr4cpp_grammar_lexer}" "${antlr4cpp_grammar_parser}"
